...
CELLROWS=7
CELLCOLS=14
out_file_maze = "name.map"
out_file_path = "name.path"
    ...
    def wander(self):
        ...
        if self.squares_to_visit == []:
            ...
            if self.first_time:
                ...
                with open(out_file_maze, 'w') as out:
        ...
        if self.next_pos == (0, 0) and not self.turn_signal and self.doing_astar:
        ...
        if self.go_left:
            ...
            if self.second_call:
                if self.move2units() == 1:
                    ...
                    self.second_call = 0
            # Small corrections on approaching walls
        ...
        if(degrees == -180 or degrees == 180):
            if self.walk == 3:
                ...
                if(direction == 'left'):
                    ...
                    self.driveMotors(0.10, -0.10)
                self.walk = 0
            elif (self.measures.compass>(180-10) and self.measures.compass<(180-4)):
                self.walk = 0
                self.driveMotors(-0.03, 0.03)
            elif (self.measures.compass>(-180+4) and self.measures.compass<(-180+10)):
                self.walk = 0
                self.driveMotors(0.03, -0.03)
            elif (self.measures.compass>=(180-4) and self.measures.compass<(180-2)):
                self.walk = 0
            ...
            if(direction == 'left'):
                ...
                self.driveMotors(0.10, -0.10)
            self.walk = 0
        elif (self.measures.compass>(degrees-10) and self.measures.compass<(degrees-4)):
            self.walk = 0
            self.driveMotors(-0.03, 0.03)
        elif (self.measures.compass>(degrees+4) and self.measures.compass<(degrees+10)):
            self.walk = 0
            self.driveMotors(0.03, -0.03)
        elif (self.measures.compass>=(degrees-4) and self.measures.compass<(degrees-2)):
            self.walk = 0
            self.driveMotors(-0.005,0.005)
        elif (self.measures.compass<=(degrees+4) and self.measures.compass>(degrees+2)):
            self.walk = 0
...
for i in range(1, len(sys.argv),2):
    if (sys.argv[i] == "--host" or sys.argv[i] == "-h") and i != len(sys.argv) - 1:
        ...
        out_file_maze = sys.argv[i + 1] + '.map'
        out_file_path = sys.argv[i + 1] + '.path'
    else:
