...
public class Plane extends Thread
{
	...
	*/
	...
	private final PlaneProxy [] passen;
	...
	*/
	...
	public Plane (GeneralReposStub repos) {
		...
		passen = new PlaneProxy [SimulPar.nPassengers];
	...
	public synchronized void boardThePlane () {   
		...
		((PlaneProxy) Thread.currentThread ()).setPassengerState (PassengerStates.INFLIGHT);
	    repos.setFlight(1);									
	    repos.setQueue(-1);
	    repos.setPassengerState (passengerId, ((PlaneProxy) Thread.currentThread ()).getPassengerState ());
	...
	*/
	...
	public synchronized void waitForAllInBoard() {
		...
		((PlaneProxy) Thread.currentThread()).setPilotState(PilotStates.WAITINGFORBOARDING);
		repos.setPilotState (((PlaneProxy) Thread.currentThread ()).getPilotState ());
		while (!allOnBoard) 
		{
			try {
	...
	*/
	...
	public synchronized void informPlaneReadyToTakeOff(int nboarded) {
		...
		((PlaneProxy) Thread.currentThread()).setHostessState(HostessStates.READYTOFLY);
		repos.setHostessState (((PlaneProxy) Thread.currentThread ()).getHostessState ());
		allOnBoard = true;
		notifyAll();
	...
	*/	
	...
	public synchronized void flyToDestinationPoint () {
        ...
        ((PlaneProxy) Thread.currentThread ()).setPilotState (PilotStates.FLYINGFORWARD);
        repos.setPilotState (((PlaneProxy) Thread.currentThread ()).getPilotState ());
	...
	*/		
	...
	public synchronized void announceArrival () {
	   ...
	   ((PlaneProxy) Thread.currentThread ()).setPilotState (PilotStates.DEBOARDING);
	   repos.setPilotState (((PlaneProxy) Thread.currentThread ()).getPilotState ());
	...
	*/	
	...
	public synchronized void leaveThePlane () {   
	    ...
	    PlaneProxy passenger = ((PlaneProxy) Thread.currentThread ()); 	
	...
	*/
	...
	public synchronized void shutdown(){
        mainPlane.waitConnection = false;
    }
